...
from imagegui import Showimage
...
def eval_one_similarity(img_df, img_dir, model, n, beta=1):
    ...
    df = pd.DataFrame(np.zeros((n_imgs, 3)))
    df.columns = ["img_idx", "imgpath", "protest"]
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if al_image is not None:
            for index, row in al_image.iterrows():
                img_name = row.fname
                image_shower= Showimage(img_dir_train+'/'+img_name)
                new_label = image_shower.label
                al_image.at[index,'protest'] = new_label
